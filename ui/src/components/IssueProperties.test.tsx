...
describe("IssueProperties", () => {
    ...
    beforeEach(() => {
        ...
        mockIssuesApi.deleteWorkProduct.mockResolvedValue(undefined);
    });
    ...
    it("shows work products from query when issue has work products", async () => {
        const workProduct = {
            id: "wp-1",
            companyId: "company-1",
            projectId: null,
        ...
        const root = renderProperties(container, {
            issue: createIssue({ workProducts: [workProduct] }),
            childIssues: [],
            onUpdate: vi.fn(),
        });
        await flush();
        ...
        const link = container.querySelector('a[href="https://github.com/example/repo/pull/42"]');
    ...
    it("shows empty state text and add button in work products section", async () => {
        mockIssuesApi.listWorkProducts.mockResolvedValue([]);
        const root = renderProperties(container, {
            issue: createIssue(),
            childIssues: [],
        ...
        expect(container.textContent).toContain("Work Products");
        expect(container.textContent).toContain("No work products yet");

        const addButton = Array.from(container.querySelectorAll("button"))
            .find((btn) => btn.title === "Add work product");
        expect(addButton).not.toBeUndefined();

        act(() => root.unmount());
    });

    it("calls createWorkProduct when the add work product form is submitted", async () => {
        mockIssuesApi.listWorkProducts.mockResolvedValue([]);
        const root = renderProperties(container, {
            issue: createIssue(),
            childIssues: [],
        ...
        await act(async () => {
            ...
            nativeInputValueSetter?.call(titleInput, "My PR");
        ...
        expect(mockIssuesApi.createWorkProduct).toHaveBeenCalledWith(
            "issue-1",
            expect.objectContaining({ title: "My PR", provider: "custom" }),
        );

...
export function Dashboard() {
    ...
    const activeGoals = useMemo(
        () => (allGoals ?? []).filter((g) => g.status === "active").slice(0, 5),
        [allGoals],
    );
                    ...
                    {activeGoals.length > 0 && (
                        <div>
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <Link to="/goals" className="text-xs text-muted-foreground hover:text-foreground transition-colors no-underline">
                                    View all
                                </Link>
                            </div>
                            <div className="border border-border divide-y divide-border overflow-hidden">
                                {activeGoals.map((goal, i) => {
                                    const progress = goalProgressQueries[i]?.data;
                                    return (
                                        <Link
                                            key={goal.id}
                                            to={`/goals/${goal.id}`}
                                            className="px-4 py-3 flex flex-col gap-2 hover:bg-accent/50 transition-colors no-underline text-inherit"
                                        >
                                            <div className="flex items-center gap-2">
                                                <span className="text-xs text-muted-foreground capitalize">{goal.level}</span>
                                                <span className="flex-1 truncate text-sm">{goal.title}</span>
                                                {progress && progress.total.total > 0 && (
                                                    <span className="text-xs text-muted-foreground shrink-0">
                                                        {progress.total.percentDone}%
                                                    </span>
                                                )}
                                            </div>
                                            {progress && progress.total.total > 0 && (
                                                <GoalProgressBar counts={progress.total} showLabel={false} compact />
                                            )}
                                        </Link>
                                    );
                                })}

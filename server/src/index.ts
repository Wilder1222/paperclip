...
export async function startServer(): Promise<StartedServer> {
    ...
    if (config.databaseUrl) {
        ...
        const getPidFilePort = (): number | null => {
            if (!existsSync(postmasterPidFile)) return null;
            try {
                const lines = readFileSync(postmasterPidFile, "utf8").split("\n");
                const pidPort = Number(lines[3]?.trim());
        ...
        const pidFilePort = getPidFilePort();
        if (runningPid) {
            if (pidFilePort && pidFilePort !== port) {
                port = pidFilePort;
            }
            logger.warn(`Embedded PostgreSQL already running; reusing existing process (pid=${runningPid}, port=${port})`);
        } else {
            const candidatePorts = [configuredPort];
            if (pidFilePort && !candidatePorts.includes(pidFilePort)) {
                candidatePorts.push(pidFilePort);
            }
            ...
            if (reusedPort !== null) {
                port = reusedPort;
                logger.warn(
                    `Embedded PostgreSQL appears reachable; reusing existing server on port ${port}`,
                );
            } else {

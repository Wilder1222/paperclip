...
/**
 * Fetch git tree entries for a given repo path.
 * When basePath is set, navigates to the specific subtree to avoid the GitHub
 * API truncation limit that affects large repositories.
 */
async function resolveGitHubSubtreeEntries(
    apiBase: string,
    owner: string,
    repo: string,
    ref: string,
    ...
    if (!basePath) {
        const tree = await fetchJson<{ tree?: GitTreeEntry[] }>(
            `${apiBase}/repos/${owner}/${repo}/git/trees/${encodeURIComponent(ref)}?recursive=1`,
        ).catch(() => ({ tree: [] as GitTreeEntry[] }));
        return tree.tree ?? [];
    }

    // Navigate each path segment to find the SHA of the target subtree so that
    // we can fetch just that subtree rather than the entire repo tree. This
    // prevents GitHub's recursive-tree API from returning a truncated response
    // when the parent repository is too large.
    const pathSegments = basePath.replace(/^\/+|\/+$/g, "").split("/").filter(Boolean);
    let currentRef = ref;
    for (const segment of pathSegments) {
        const levelTree = await fetchJson<{ tree?: GitTreeEntry[] }>(
            `${apiBase}/repos/${owner}/${repo}/git/trees/${encodeURIComponent(currentRef)}`,
        ).catch(() => ({ tree: [] as GitTreeEntry[] }));
        const treeEntry = (levelTree.tree ?? []).find((e) => e.path === segment && e.type === "tree");
        if (!treeEntry) return [];
        currentRef = treeEntry.sha;
    }

    const subtree = await fetchJson<{ tree?: Array<{ path: string; type: string }> }>(
        `${apiBase}/repos/${owner}/${repo}/git/trees/${encodeURIComponent(currentRef)}?recursive=1`,
    ).catch(() => ({ tree: [] as Array<{ path: string; type: string }> }));

    const prefix = `${basePath.replace(/^\/+|\/+$/g, "")}/`;
    return (subtree.tree ?? []).map((e) => ({ ...e, path: `${prefix}${e.path}` }));
}
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
    ...
    async function resolveSource(source: CompanyPortabilityPreview["source"]): Promise<ResolvedSource> {
        ...
        const treeEntries = await resolveGitHubSubtreeEntries(
            apiBase,
            parsed.owner,
            parsed.repo,
            ref,
        ...
        const candidatePaths = treeEntries

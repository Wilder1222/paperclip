...
function buildMemoryRecallQuery(input: {
    ...
    const parts = [
        readNonEmptyString(input.issueTitle),
        readNonEmptyString(input.issueDescription),
        readNonEmptyString(input.wakeCommentBody),
        readNonEmptyString(input.continuationSummaryBody),
        readNonEmptyString(input.taskMarkdown),
    ]
...
function summarizeMemoryRecallItemsForContext(items: Array<{ id: string; content: string; metadata: Record<string, unknown> }>) {
...
export function heartbeatService(db: Db, options: HeartbeatServiceOptions = {}) {
    ...
    async function ingestContinuationSummaryForRun(input: {
        resolvedMemoryProvider: ResolvedMemoryProvider | null;
        run: typeof heartbeatRuns.$inferSelect;
        issueId: string | null;
        issueTitle: string | null;
        ...
        if (!input.resolvedMemoryProvider) return;
        const summaryBody = readNonEmptyString(input.summaryBody);
        if (!summaryBody) return;

        const ingestStartedAt = Date.now();
        try {
            const content = [
                input.issueId ? `issueId: ${input.issueId}` : null,
                input.issueTitle ? `issueTitle: ${input.issueTitle}` : null,
                "summary:",
                summaryBody,
            ]
            ...
            await input.resolvedMemoryProvider.provider.ingest(content, {
                projectId: input.resolvedMemoryProvider.namespaceProjectId,
                runId: input.run.id,
                issueId: input.issueId ?? undefined,
            });

            await recordMemoryOperation(db, {
                companyId: input.run.companyId,
                bindingId: input.resolvedMemoryProvider.binding.id,
    ...
    async function executeRun(runId: string) {
        ...
        try {
            ...
            const sessionCodec = getAdapterSessionCodec(agent.adapterType);
            const issueId = readNonEmptyString(context.issueId);
            let resolvedMemoryProvider: ResolvedMemoryProvider | null = null;
            ...
            resolvedMemoryProvider = await resolveActiveMemoryProvider(db, {
                companyId: agent.companyId,
                projectId: executionProjectId,
                agentId: agent.id,
            });
            const memoryRecallQuery = buildMemoryRecallQuery({
                issueTitle: issueRef?.title,
                issueDescription: issueRef?.description,
                wakeCommentBody: wakeCommentContext?.body,
                continuationSummaryBody: continuationSummary?.body,
                taskMarkdown,
            });
            if (resolvedMemoryProvider && memoryRecallQuery) {
                const recallStartedAt = Date.now();
                try {
                    const recallResult = await resolvedMemoryProvider.provider.recall({
                        query: memoryRecallQuery,
                        projectId: resolvedMemoryProvider.namespaceProjectId,
                        topK: 5,
                    });
                    const recallItems = summarizeMemoryRecallItemsForContext(recallResult.items);
                    if (recallItems.length > 0) {
                        context.paperclipMemoryRecall = {
                            providerType: resolvedMemoryProvider.binding.providerType,
                            scopeType: resolvedMemoryProvider.binding.scopeType,
                            itemCount: recallItems.length,
                            projectId: resolvedMemoryProvider.namespaceProjectId,
                            items: recallItems,
                        };
                    } else {
                        delete context.paperclipMemoryRecall;
                    }
                    await recordMemoryOperation(db, {
                        companyId: agent.companyId,
                        bindingId: resolvedMemoryProvider.binding.id,
            ...
            try {
                ...
                if (finalizedRun) {
                    ...
                    const refreshedContinuationSummary = await refreshContinuationSummaryForRun(livenessRun, agent);
                    await ingestContinuationSummaryForRun({
                        resolvedMemoryProvider,
                        run: livenessRun,
                        issueId,
                        issueTitle: issueRef?.title ?? null,
                        summaryBody: refreshedContinuationSummary?.body ?? continuationSummary?.body ?? null,
                        agentId: agent.id,
                    });
